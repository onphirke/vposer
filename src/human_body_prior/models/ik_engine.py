...
def ik_fit(
    ...
    def fit(weights, free_vars):
        ...
        free_vars["pose_body"] = (
            vp_model.decode(free_vars["poZ_body"])["pose_body"]
            .contiguous()
            .view(-1, 63)
        )
...
class IK_Engine(nn.Module):
    ...
    def forward(self, source_kpts, target_kpts, initial_body_params={}):
        ...
        initial_body_params["poZ_body"] = self.vp_model.encode(
            initial_body_params["pose_body"]
        ).mean

        free_vars = {
            k: torch.nn.Parameter(v.detach(), requires_grad=True)
            for k, v in initial_body_params.items()
            if k in ["betas", "trans", "poZ_body", "root_orient"]
        }
        static_vars = {
