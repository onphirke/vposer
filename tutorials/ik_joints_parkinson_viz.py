...
try:
    ...
    print(
        "Warning: body_visualizer not available. Using basic matplotlib visualization."
    )
...
def create_body_meshes(
    results: List[Tuple[Dict, float]], bm: BodyModel, device: torch.device
) -> List[Dict]:
    """
...
def create_joint_video_matplotlib(
    body_meshes: List[Dict], output_path: str, joint_mapping: Dict
) -> None:
    """
    ...
    def animate(frame_idx):
        ...
        for connection in skeleton_connections:
            ...
            if start_idx < len(joints) and end_idx < len(joints):
                ax.plot(
                    [joints[start_idx, 0], joints[end_idx, 0]],
    ...
    anim = animation.FuncAnimation(
        fig, animate, frames=n_frames, interval=200, repeat=True, blit=False
    )
...
def create_mesh_video_advanced(
    ...
    print(
        f"Creating high-quality mesh video with batch rendering (batch_size={batch_size})..."
    )
    ...
    try:
        ...
        def rotation(batch_idx):
            # return [0, batch_idx * 25, 0]
            # return [batch_idx * 25, 0, 0]
            return [-90, -20, 0]

        first_frame = render_smpl_params(bm, first_batch_params, rot_body=rotation(0))
        ...
        for batch_idx, batch_start in enumerate(
            range(frames_written, n_frames, batch_size)
        ):
            batch_end = min(batch_start + batch_size, n_frames)
            current_batch_size = batch_end - batch_start

            print(
                f"Rendering batch {batch_idx + 1}/{n_batches}: frames {batch_start + 1}-{batch_end} ({current_batch_size} frames)"
            )
            ...
            batch_images = render_smpl_params(bm, batch_params, rot_body=rotation(batch_idx))
            ...
            print(
                f"  Completed batch {batch_idx + 1}/{n_batches} ({frames_written} total frames written)"
            )
            ...
            if IMAGEIO_AVAILABLE:
                with imageio.get_writer(output_path, mode="I", fps=FPS) as writer:
                    ...
                    for batch_idx, batch_start in enumerate(
                        ...
                        batch_images = render_smpl_params(
                            bm, batch_params, rot_body=[-90, 0, 0]
                        )
                        if isinstance(batch_images, torch.Tensor):
                            batch_images = batch_images.cpu().numpy()
...
def analyze_joint_movement(
    body_meshes: List[Dict], joint_mapping: Dict, output_dir: str
) -> None:
    """
    ...
    key_joints = {"Head": 15, "Left Wrist": 20, "Right Wrist": 21, "Pelvis": 0}
...
def main():
    ...
    print(
        f"  - Movement analysis: {osp.join(output_dir, 'joint_movement_analysis.png')}"
    )
    if BODY_VISUALIZER_AVAILABLE:
        print(
            f"  - Advanced mesh video: {osp.join(output_dir, 'parkinson_motion_mesh.mp4')}"
        )
