...
import numpy as np
import torch
from body_visualizer.tools.vis_tools import imagearray2file, render_smpl_params
...
from human_body_prior.tools.omni_tools import create_list_chunks, get_support_data_dir
...
def convert_mdm_mp4_to_amass_npz(
    skeleton_movie_fname,
    out_fname=None,
    save_render=False,
    comp_device="cuda:0",
    ...
    support_dir = osp.join(
        support_base_dir, "dowloads"
    )  #'../../../support_data/dowloads'
    ...
    if isinstance(skeleton_movie_fname, np.ndarray):
        assert out_fname is not None, (
            "when passing motion file out_fname should be provided"
        )
        motion = skeleton_movie_fname
    else:
        assert osp.exists(skeleton_movie_fname), skeleton_movie_fname
        parsed_name = (
            osp.basename(skeleton_movie_fname)
            .replace(".mp4", "")
            .replace("sample", "")
            .replace("rep", "")
        )
        ...
        motion = mdm_data["motion"][absl_idx].transpose(
            2, 0, 1
        )  # [nframes, njoints, 3]
    ...
    if osp.exists(out_fname):
        ...
        for cur_frame_ids in batched_frames:
            target_pts = torch.from_numpy(motion[cur_frame_ids, :n_joints]).to(
                comp_device
            )
        ...
        d["poses"] = np.concatenate(
            [d["root_orient"], d["pose_body"], np.zeros([len(d["pose_body"]), 99])],
            axis=1,
        )
    ...
    if save_render:
        ...
        body_parms = {
            **d,
            "betas": np.repeat(d["betas"][None], axis=0, repeats=len(motion)),
    ...
    logger.info(
        f"You can visualize these results as any amass npz file or in Blender via blender_smplx_addon."
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input",
        type=str,
        help="skeleton movie.mp4 filename that is to be converted into SMPL",
    )
    parser.add_argument(
        "--pattern",
        type=str,
        help="filename pattern for skeleton */*/movies.mp4 to be converted into SMPL",
    )
    parser.add_argument(
        "--batch_size", type=int, default=128, help="batch size for inverse kinematics"
    )
    parser.add_argument(
        "--model_type", type=str, default="smplx", help="model_type; e.g. smplx/smpl"
    )
    parser.add_argument(
        "--device", type=str, default="cuda:0", help="computation device"
    )
    parser.add_argument(
        "--gender", type=str, default="neutral", help="gender; e.g. neutral"
    )
    parser.add_argument(
        "--save_render", type=bool, default=True, help="render IK results"
    )
    parser.add_argument(
        "--verbosity", type=int, default=0, help="0: silent, 1: text, 2: display"
    )
    ...
    if not params.input is None:
        convert_mdm_mp4_to_amass_npz(
            skeleton_movie_fname=params.input,
        ...
        for mp4_fname in mp4_fnames:
            ...
            convert_mdm_mp4_to_amass_npz(
                skeleton_movie_fname=mp4_fname,
                surface_model_type=params.model_type,
                gender=params.gender,
                batch_size=params.batch_size,
                save_render=params.save_render,
